...
Good: "Run `bun install jsonwebtoken`, then edit `src/auth.ts:42`."
...
3. Run `bun run test -- auth.spec.ts`
```
...
Good: "Next: run `bun run test` and paste the first failing line."
...
Good: "Login now works with magic links. Try: `bun run dev`, open `/login`."
